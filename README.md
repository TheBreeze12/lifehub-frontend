...
| 餐后散步30分钟              | 指定运动类型和时长 |
| 公园健走                    | 指定运动地点和类型 |
| 今天午餐后在北京安排散步    | 指定时间和地点     |

#### 天气查询

运动计划详情页面会自动显示天气信息，包括：
- 当前温度
- 风速和风向
- 天气状况
- 小时级天气预报
...
- 检查昵称和密码是否正确
- 确认后端服务正常运行
- 查看后端日志了解详细错误（用户不存在/密码错误）

### Q10: 天气查询失败
**A**: 
- 确认运动计划包含目的地或坐标信息
- 检查网络连接
- 天气服务使用Open-Meteo，无需API Key，但需要网络连接
